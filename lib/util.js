...
const Util = module.exports = {
  ...
  mergeApplicationConfig (app, pack) {
    ...
    return _.mergeWith(app.config, packConfig, (appValue, packValue, key, o, s, stack) => {
      ...
      if (conflicts.length > 0) {
        _.each(conflicts, conflictKey => {
          const packConflict = packValue[conflictKey]
          const appConflict = appValue[conflictKey]

          if (_.isArray(appConflict)) {
            // warn if the two config values are not of compatible types
            if (!_.isArray(packConflict)) {
              app.log.warn('The config setting', key, 'in the application is an',
                'array, but that setting in', packName, 'is not an array.')
            }
